...
//operations
 ...
 {  
    ...
    if(ret != 0)
    {
            pr_info("%s pchar_open() unblocked due to signal for pchar%d\n",THIS_MODULE->name,dev->id);

    }
    pr_info("pchar%d blocked for pchar_open\n",dev->id);
...
static ssize_t pchar_write(struct file *pfile, const char * __user ubuf, size_t bufsize, loff_t *pf_pos)
{
    ...
    if(ret < 0)
    {
        pr_info("%s kfifo_from_user() failed for pchar%d\n",THIS_MODULE->name,dev->id);
        return ret;
    }
    pr_info("%s pchar_write() written %d bytes on pchar%d\n",THIS_MODULE->name,nbytes,dev->id);
    return nbytes;
}
static ssize_t pchar_read(struct file *pfile, char * __user ubuf, size_t bufsize, loff_t *pf_pos)
{
    pchardev_t *dev = (pchardev_t *)pfile->private_data;
    int ret,nbytes;
    pr_info("%s pchar_read() called by pchar%d\n",THIS_MODULE->name,dev->id);
    ret = kfifo_to_user(&dev->mybuf,ubuf,bufsize,&nbytes);
    if(ret < 0)
    {
        pr_info("%s kfifo_to_user() failed for pchar%d\n",THIS_MODULE->name,dev->id);
        return ret;
    }
    pr_info("%s pchar_read() read %d bytes from pchar%d\n",THIS_MODULE->name,nbytes,dev->id);
    return nbytes;
}
...
//alllocate array to private device structure
...
major = MAJOR(devno);
pr_info("%s alloc_chrdev_region() allocated device no %d\n",THIS_MODULE->name,major);
// device class
pclass = class_create("pchar_class");
if(IS_ERR(pclass))
{
    pr_info("%s class_create() failed for char%d\n",THIS_MODULE->name,i);
    ret =-1;
...
for(i=0;i < DEVCNT;i++)
{
   devnum = MKDEV(major,i);
    ...
    if(IS_ERR(pdevice))
    {
        pr_info("%s device_create() failed for pchar%d\n",THIS_MODULE->name,i);
        ret =-1;
        goto device_create_failed;
    }
    pr_info("%s device_create() created device file pchar%d\n",THIS_MODULE->name,i);
}
//cdev intializatin,and add to kernel database
for(i=0;i < DEVCNT;i++)
{
    devnum = MKDEV(major,i);
    devices[i].cdev.owner = THIS_MODULE;//accesing array of structure
    cdev_init(&devices[i].cdev,&pchar_fops);
    ret = cdev_add(&devices[i].cdev,devnum,1);  
    if(ret != 0)
    {
        pr_info("%s cdev_add() failed for pchar%d\n",THIS_MODULE->name,i);
        goto cdev_add_failed;
    }  

    pr_info("%s cdev_add() added cdev in kernel pchar%d\n",THIS_MODULE->name,i);

}

//intializing kfifo of each device
for(i =0 ;i<DEVCNT;i++)
{       
    devices[i].id = i;
    ...
    if(ret != 0)
    {
        pr_info("%s kfifo_alloc() failed for pchar%d\n",THIS_MODULE->name,i);
...
{       
    ...
    pr_info("%s mutex_init() intialised mutex for pchar%d\n",THIS_MODULE->name,i);
}
//all intializatin sucessful
return 0;
...
//deinitialize device info
    ...
    {
        kfifo_free(&devices[i].mybuf);
        pr_info("%s kifo_free() released fifo for pchar%d \n",THIS_MODULE->name,i);
...
{
    cdev_del(&devices[i].cdev);
    pr_info("%s cdev_del() deleted cdev from kernel for pchar%d\n",THIS_MODULE->name,i);
...
for(i =0;i< DEVCNT;i++)
{
    ...
    pr_info("%s device_destroy() destroy device file for pchar%d\n",THIS_MODULE->name,i);
}
...
unregister_chrdev_region(devno,DEVCNT);
pr_info("%s unregister_chrdev_region() released device major no %d\n",THIS_MODULE->name,major);
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("multiple pseduo char device");
MODULE_AUTHOR("p2 < anonymous@gmail.com>");
